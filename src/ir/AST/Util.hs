...
extendMProgram f (p@Program{functions, classes}) =
    ...
    where
      ...
      extendMFunction f (fun@MatchingFunction{matchfunbodies}) =
          do matchfunbodies' <- mapM (extendM f) matchfunbodies
             return fun{matchfunbodies = matchfunbodies'}
      ...
      freeVariables' bound Let {decls, body} =
          ...
          where
            ...
            fvDecls (vars, expr) (free, bound) =
              let xs = map varName vars
              in (freeVariables' (xs ++ bound) expr ++ free, xs ++ bound)
